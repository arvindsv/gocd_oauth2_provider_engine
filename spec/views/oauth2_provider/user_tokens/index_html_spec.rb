...
module Oauth2Provider
  describe 'oauth2_provider/user_tokens/index.html.erb' do
    ...
    it "should display message when there are no tokens" do
      ...
      expect(response).to have_selector(".content_wrapper_inner .info-box", :text => "There are no tokens.")
    end
    ...
    it "should display tokens when present" do
      ...
      Capybara.string(response.body).all("table#oauth_user_token_table tr").tap do |rows|
        rows[1].all("td").tap do |columns|
          expect(columns[0].text).to eq(token_1.oauth_client.name)
          expect(columns[1]).to have_selector("a.delete_link[@href='/revoke/token_1']", :text => "Delete")
        end

        rows[2].all("td").tap do |columns|
          expect(columns[0].text).to eq(token_2.oauth_client.name)
          expect(columns[1]).to have_selector("a.delete_link[@href='/revoke/token_2']", :text => "Delete")
        end
      end
    end
  end
